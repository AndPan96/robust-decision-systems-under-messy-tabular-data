...
def update_rows(updates: dict):
    ...
    if "application_train" in updates:
        ...
        df = pd.read_csv(RAW_APPLICATION_TRAIN, low_memory=False).set_index("SK_ID_CURR")
        updates_df = pd.DataFrame(updates["application_train"]).set_index("SK_ID_CURR")
        updates_df.replace({None: np.nan})
        updates_df = updates_df.astype(df.dtypes.to_dict(), errors="ignore")

        new_idx = updates_df.index.difference(df.index)
        up_idx = updates_df.index.intersection(df.index)

        if not updates_df.loc[new_idx].empty: 
        ...
        if (
            PROCESSED_IDS.exists() and
            PROCESSED_PREDS.exists() and
            PROCESSED_X.exists()
        ):
            ...
            if len(proc_ids) > 0:
                ...
                proc_mask = ~proc_ids["SK_ID_CURR"].isin(no_target_idx).to_numpy()
                proc_ids.iloc[proc_mask].to_parquet(PROCESSED_IDS, index=False)
                proc_preds.iloc[proc_mask].to_parquet(PROCESSED_PREDS, index=False)
                proc_x.iloc[proc_mask].to_parquet(PROCESSED_X, index=False)
    ...
    for table in updates:
        ...
        if not updates_df[new_idx].empty:
